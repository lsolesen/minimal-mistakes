...
Post and page excerpts are auto-generated grapping the first paragraph of text. To override this text with something more specific use the following YAML Front Matter:
...
![transparent custom overlay]({{ "/assets/images/mm-header-overlay-custom-filter.jpg" }})
...
To assign more author links, add to the `author.links` array  in [`_config.yml`]({{ "/docs/configuration/" }}) link so. Any of [Font Awesome's icons](https://fontawesome.com/v6/search) are available for use.
...
```html
{% raw %}<a href="https://www.reddit.com/submit?url={{ page.url | htmlBaseurl: site.url | url_encode }}&title={{ page.title }}" class="btn" title="{{ ui-text[site.locale].share_on_label }} Reddit"><i class="fab fa-fw fa-reddit" aria-hidden="true"></i><span> Reddit</span></a>{% endraw %}
```
...
```html
{% raw %}<a href="https://www.reddit.com/submit?url={{ page.url | htmlBaseUrl: site.url | url_encode }}&title={{ page.title }}" class="btn btn--reddit" title="{{ ui-text[site.locale].share_on_label }} Reddit"><i class="fab fa-fw fa-reddit" aria-hidden="true"></i><span> Reddit</span></a>{% endraw %}
```
