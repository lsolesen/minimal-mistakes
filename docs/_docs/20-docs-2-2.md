...
The preferred method for running the site is with `npm run watch:eleventy`.
...
How Minimal Mistakes is organized and what the various files are. All posts, layouts, includes, stylesheets, assets, and whatever else is grouped nicely under the root folder. The compiled 11ty site outputs to `_site/`.
...
`_data/site.yml` is your friend. Open it up and personalize it. Most variables are self explanatory but here's an explanation of each if needed:
